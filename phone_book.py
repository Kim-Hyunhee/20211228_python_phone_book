...
# 몇 번째 연락처를 지울 건지 -> 그 위치의 연락처 삭제
def remove_contact_by_position():
    # 지우고 싶은 위치를 입력받자
    position = int(input('몇 번째 연락처를 삭제하겠습니까? (1부터 시작) : '))
    # 그 위치의 연락처가 누구인지? 이름 => 확인받자
    ...
    with open ('phone_book.csv', 'r') as f:
        ...
        # 범위를 벗어나는 위치 입력 => 해당 위치의 연락처는 없습니다. + 메뉴로 돌아가게 설정
        if position >= len(contact_list)  or position < 0 :
            print('해당 위치의 연락처는 없습니다.')
            # 연락처 삭제 기능 (함수 or 메쏘드) 강제 종료
            return  # 이 함수의 결과가 여기서 나온다고 지정 ( 결과가 따로 없다 ) => 밑의 코드들은 실행되지 않게 처리
    ...
    confirm = input(f'정말 {remove_contact.name} 연락처를 삭제하겠습니다? (y/n) : ') 
    
    # y로 실제 삭제만 코딩
    if confirm == 'y':
        
        # 지울 사람만 빼고, 나머지 인원들이 추가 되도록 파일을 새로 작성
        with open ('phone_book.csv', 'w') as f:
            # 읽기 모드에서 가져온 연락처 정보 (이름, 폰번, 메모) str 을 불러내보자
            for line in contact_list:
                # line - '이름, 폰번,메모\n' 정보가 들어있을 예정
                # remove_contact의 폰번 => 불러낸 line이 포함하고 있는가? 맞으면 삭제 (추가 X)
